...
## Vanessa Editor
   ...
   * **ПриводитьОтступыПриСохранении {normalizeindentationonsave}**:
      При сохранении файла в редакторе отступы в начале всех строк приводятся к одному виду: к пробелам, если включена опция "Заменять табы на пробелы", иначе к табуляции.

   * **РазмерТабуляции {tabsize}**:
      Количество пробелов, соответствующее одному символу табуляции в редакторе. По умолчанию 4.
