...
Процедура СохранитьТекстИзVanessaEditorВФайл() Экспорт
	Если Ванесса.Объект.ПриводитьОтступыПриСохранении Тогда
		УстановитьЗаменуТабуляцииНаПробелы();
		ПолучитьVanessaEditor().normalizeIndentation();
	КонецЕсли;
...
// Устанавливает надо ли заменять символы табуляции на пробелы при работе в редакторе и размер табуляции
&НаКлиенте
Процедура УстановитьЗаменуТабуляцииНаПробелы() Экспорт
	Попытка
		Если Ванесса.VanessaTabs.current <> Неопределено Тогда
			Если Ванесса.ЭтоВкладкаEdit(Ванесса.VanessaTabs.current) Тогда
				ПолучитьVanessaEditor().setTabSize(РазмерТабуляцииРедактора());
				Если Ванесса.Объект.ЗаменятьТабыНаПробелы Тогда
					ПолучитьVanessaEditor().setInsertSpaces(Истина);
				Иначе
					ПолучитьVanessaEditor().setInsertSpaces(Ложь);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	Исключение
...
// Возвращает размер табуляции для редактора
&НаКлиенте
Функция РазмерТабуляцииРедактора() Экспорт
	Если Ванесса.Объект.РазмерТабуляции > 0 Тогда
		Возврат Ванесса.Объект.РазмерТабуляции;
	КонецЕсли;
	Возврат 4;
КонецФункции
